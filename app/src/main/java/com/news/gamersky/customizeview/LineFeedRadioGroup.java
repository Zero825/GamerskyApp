...
//https://blog.csdn.net/shaoyezhangliwei/article/details/75278001

public class LineFeedRadioGroup extends RadioGroup {
    private static final String TAG="LineFeedRadioGroup";

    private View layout;
    private int height;
    private ValueAnimator showOrHideAnimator;
    ...
    public void init(){
        layout=this;
        showOrHideAnimator=new ValueAnimator();
...
//        });
    ...
    public void showOrHide(final boolean show){
        Log.i(TAG, "showOrHide: "+layout.getMeasuredHeight()+"\t"+height);
        showOrHideAnimator=show?ValueAnimator.ofInt(layout.getMeasuredHeight(),height)
                :ValueAnimator.ofInt(layout.getMeasuredHeight(),0);
        showOrHideAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                setLayoutHeight(layout,(int)animation.getAnimatedValue());
            }
        });
        showOrHideAnimator.setDuration(300);
        showOrHideAnimator.start();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(!showOrHideAnimator.isRunning()){
            height=totalHeight;
        }
    }
